...
class PointCloudCrop(Node):
    ...
    def _detections_callback(self, msg):
        ...
        for index, detection in enumerate(msg.detections):
            ...
            current_marker_ids.add(index)

        # A previous message may have produced markers this one didn't (either fewer
        # detections, or this one's correction failed) -- explicitly delete those
        # now-unused ids so their boxes don't linger past marker_lifetime.
        for stale_id in self.previous_marker_ids - current_marker_ids:
