    ...
    arguments = [
        ...
        DeclareLaunchArgument(
            ...
            description='Left-arm grasp height (base_link Z), same fixed height as '
                        'bottle_box -- measured via tf2_echo base_link '
        ...
        DeclareLaunchArgument(
            ...
            description='y offset (0.0 = no correction). Valid AT '
                        'grasp_position_y_reference_pixel -- see grasp_position_y_slope '
                        'below.',
        ),
        DeclareLaunchArgument(
            'grasp_position_y_slope',
            default_value='0.00010416666666666666',
            description='A constant y offset only ever matches one screen position -- '
                        'real-robot testing showed the box needs progressively more '
                        '+Y (left) correction as its detected pixel moves from the '
                        'left side of frame toward center: 0.0 at pixel_u=288 '
                        '("left"), +0.03 (3cm left) at pixel_u=576 ("center"). '
        ...
        # --- Place sequence (after lifting) -> release -> back to hover -> home ---
        DeclareLaunchArgument(
            'place_hover_position_xyz',
            default_value='[0.32607388496398926, 0.13636542856693268, 0.8921283483505249]',
            description='Hover pose above the drop location, moved to right after '
                        'lifting. Measured via `ros2 topic echo --once /l_goal_pose` '
                        'with the arm holding a box there.',
        ),
        DeclareLaunchArgument(
            'place_hover_orientation_xyzw',
            default_value='[-0.019953999668359756, -0.6819889545440674, '
                           '-0.020979225635528564, 0.7307890057563782]',
        ),
        DeclareLaunchArgument('place_hover_duration', default_value='4.0'),
        DeclareLaunchArgument(
            'place_position_xyz',
            default_value='[0.6525217294692993, 0.11738879978656769, 0.8008310198783875]',
            description='Actual release pose, moved to from place_hover. Same '
                        'measurement method.',
        ),
        DeclareLaunchArgument(
        ...
        DeclareLaunchArgument(
            'return_to_initial',
            default_value='true',
            description='Return to the initial pose after releasing and backing back '
                        'to place hover.',
        ),
        DeclareLaunchArgument(
            'home_position_xyz',
